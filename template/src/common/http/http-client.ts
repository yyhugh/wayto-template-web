...
export default class HttpClient
{
     ...
     */
    private getAxiosRequest(method: string, request: IHttpRequest): AxiosRequestConfig
    {
        ...
        let requestUrl = Type.isUndefined(request.urlPath) ?  request.url : tools.formatString(request.url, request.urlPath);
        ...
        // 构造服务器响应的数据类型。
        let responseType = !Type.isUndefined(request.responseType) && request.responseType;
        ...
        // 设置进度回调函数
        if(Type.isFunction(request.progress))
        {
            config.onUploadProgress = request.progress;
        }
    ...
    {
        const response = this.resolveResponse(axiosResponse, axiosRequest);
    ...
    private resolveResponse(axiosResponse: AxiosResponse, axiosRequest: AxiosRequestConfig): IHttpResponse
    {
        ...
        {
            code: typeof content.code === "number" ? content.code : code,
            request,
            content: data,
            extras,
            headers
