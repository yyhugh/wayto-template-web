<template>
    <i-header class="layout-header">
        <h1 class="layout-header-textlogo" @click="jumpTo('home')"><%_ if (options.chineseName) { _%> <%= options.chineseName %> <%_ } _%></h1>
        <aside class="layout-header-side">
            <i-row class="layout-header-user" type="flex" justify="end" align="middle">
                <i-avatar class="user-avatar ml20" :src="require('assets/default_boy.png')"></i-avatar>
                <i-dropdown trigger="click" placement="bottom-end" @on-click="onDropdownClick">
                    <a class="username" href="javascript:void(0)">
                        <span>欢迎您，系统管理员</span>
                        <i class="iconfont iconicon_drop-down"></i>
                    </a>
